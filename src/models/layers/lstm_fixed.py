...
class MyLSTM(nn.Module):
    ...
    def __init__(
        ...
        output_dims = [hidden_size] * num_layers
        self.fwd_cells = nn.ModuleList([
            MyLSTMCell(input_size=di, hidden_size=do)
            for di, do in zip(input_dims, output_dims)
        ])
        if self.bidirectional:
            self.bwd_cell = nn.ModuleList([
                MyLSTMCell(input_size=di, hidden_size=do)
                for di, do in zip(input_dims, output_dims)
            ])
    ...
    def transduce_layer(self, layer, inputs, hl0, cl0, lengths=None, forward=True):
        # Sentence lengths
        L, bsz = inputs.size()
        ...
        hls, cls = [], []
        for i in range(L):
            # Mask hidden state when doing bwd transduction
    ...
    def transduce(self, inputs, h0_fwd, h0_bwd, lengths=None, forward=True):
        ...
        for layer in range(self.num_layers):
            hl0, cl0 = h0[layer], c0[layer]
            hl_fwd, _ = self.transduce_layer(
                layer, hl, hl0, cl0, lengths)
        for i in range(L):
            hs_i, cs_i = [], []
            # Reset the state
            if lengths is not None and not forward and i <= L - min_length:
